	...
	*/*;q=0.8",
    ...
    while pag < 20:
    	for proxy in shuffled_proxies: # vamos variando el proxy
        	...
        	while responde and proxi_cons_request < 2 :# mientras el proxy responda y le hallamos hecho menos de 2 peticiones
        		try:
        			time.sleep(randint(0, 10)) #espera aleatoria entre 0 y 10 segundos
        			#pedimos la pagina deseada
        			web = requests.get( "https://www.amazon.es/s?k=m%C3%B3viles&i=electronics&rh=n%3A599370031%2Cn%3A665492031%2Cn%3A17425698031&dc&page="+str(pag_ini+pag)+"&__mk_es_ES=%C3%85M%C3%85%C5%BD%C3%95%C3%91&qid=1603228538&rnid=599370031&ref=sr_pg_2", headers = headers, proxies = {'http':proxy,'https': proxy}, timeout=2 )
	...
	*/*;q=0.8",
    ...
    while enlace < 800:
    	for proxy in shuffled_proxies:
    		...
    		while responde and enlace < 800 : # en este caso solo quiero una petición cada vez
    			try:
...
# esta función lanza 14 hebras que ejecutarán la función "obtener_links_hebra" en paralelo.
